...
namespace reshade
{
	namespace fx
	{
		...
		struct parser
		{
			parser(const std::string &input, nodetree &ast, std::string &errors) : _ast(&ast), _errors(&errors), _lexer(new lexer(input))
			{
			}
			...
			// Expressions
			bool accept_unary_op(enum unary_expression_node::op &op)
			{
				switch (_token_next.id)
				{
					case lexer::tokenid::exclaim:
			...
			bool accept_postfix_op(enum unary_expression_node::op &op)
			{
				switch (_token_next.id)
				{
					case lexer::tokenid::plus_plus:
			...
			bool peek_multary_op(enum binary_expression_node::op &op, unsigned int &precedence) const
			{
				switch (_token_next.id)
				{
					case lexer::tokenid::percent:
			...
			bool accept_assignment_op(enum assignment_expression_node::op &op)
			{
				switch (_token_next.id)
				{
					case lexer::tokenid::equal:
			...
			bool parse_expression(expression_node *&node)
			{
			...
			bool parse_expression_unary(expression_node *&node)
			{
				type_node type;
			...
			bool parse_expression_multary(expression_node *&left, unsigned int left_precedence = 0)
			{
			...
			bool parse_expression_assignment(expression_node *&left)
			{
			...
			// Statements
			bool parse_statement(statement_node *&statement, bool scoped = true)
			{
...
#pragma endregion
			...
			bool parse_statement_block(statement_node *&statement, bool scoped = true)
			{
				if (!expect('{'))
				{
					return false;
			...
			bool parse_statement_declarator_list(statement_node *&statement)
			{
				type_node type;
			...
			bool parse_annotations(std::vector<annotation_node> &annotations)
			{
				if (!accept('<'))
				{
					return false;
			...
			bool parse_struct(struct_declaration_node *&structure)
			{
			...
			bool parse_function_residue(type_node &type, std::string name, function_declaration_node *&function)
			{
			...
			bool parse_variable_residue(type_node &type, std::string name, variable_declaration_node *&variable, bool global = false)
			{
			...
			bool parse_variable_assignment(expression_node *&expression)
			{
				if (accept('{'))
				{
			...
			bool parse_variable_properties(variable_declaration_node *variable)
			{
				if (!expect('{'))
				{
					return false;
			...
			bool parse_variable_properties_expression(expression_node *&expression)
			{
				backup();
			...
			bool parse_technique(technique_declaration_node *&technique)
			{
			...
			bool parse_technique_pass(pass_declaration_node *&pass)
			{
			...
			bool parse_technique_pass_expression(expression_node *&expression)
			{
				scope scope;
				bool exclusive;
			...
			expression_node *fold_constant_expression(expression_node *expression) const
			{
