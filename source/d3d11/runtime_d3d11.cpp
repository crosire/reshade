	...
	_app_state(device)
{
	...
	subscribe_to_load_config([this](const ini_file &config) {
		...
		config.get("DX11_BUFFER_DETECTION", "DepthBufferHiddenByRectangleNumber", _depth_hidden_by_rectangle_index_override);
	...
	subscribe_to_save_config([this](ini_file &config) {
		...
		config.set("DX11_BUFFER_DETECTION", "DepthBufferRetrievalMode", detection_mode);
...
bool reshade::d3d11::runtime_d3d11::init_effect(size_t index)
{
	...
	for (technique &technique : _techniques)
	{
		...
		auto impl = new d3d11_technique_data(technique_init);
		technique.impl = impl;
...
void reshade::d3d11::runtime_d3d11::unload_effect(size_t index)
{
	for (technique &tech : _techniques)
	{
		...
		delete static_cast<d3d11_technique_data *>(tech.impl);
		tech.impl = nullptr;
	}
...
{
	auto impl = static_cast<d3d11_tex_data *>(texture.impl);
...
{
	delete static_cast<d3d11_tex_data *>(texture.impl);
	texture.impl = nullptr;
}

void reshade::d3d11::runtime_d3d11::render_technique(technique &technique)
{
	const auto impl = static_cast<d3d11_technique_data *>(technique.impl);
	d3d11_effect_data &effect_data = _effect_data[technique.effect_index];
...
#if RESHADE_DEPTH
void reshade::d3d11::runtime_d3d11::draw_depth_debug_menu(buffer_detection_context &tracker)
{
	...
	for (const auto &[dsv_texture, snapshot] : tracker.depth_buffer_counters())
	{
		...
		if (_preserve_hidden_depth_buffers && dsv_texture == tracker.current_hidden_by_rectangle_depth_texture())
		{
			for (UINT hidden_depth_buffer_index = 1; hidden_depth_buffer_index <= snapshot.rectangles.size(); ++hidden_depth_buffer_index)
			{
				...
				if (bool value = (_depth_hidden_by_rectangle_index_override == hidden_depth_buffer_index);
					ImGui::Checkbox(label, &value))
				{
					_depth_hidden_by_rectangle_index_override = value ? hidden_depth_buffer_index : std::numeric_limits<UINT>::max();
					modified = true;
				}
				...
				ImGui::Text("%*s|           | %5u draw calls ==> %8u vertices |",
					sizeof(dsv_texture.get()) == 8 ? 8 : 0, "", // Add space to fill pointer length
					snapshot.rectangles[hidden_depth_buffer_index - 1].drawcalls, snapshot.rectangles[hidden_depth_buffer_index - 1].vertices);
			}
		}
...
void reshade::d3d11::runtime_d3d11::update_depth_texture_bindings(com_ptr<ID3D11Texture2D> depth_texture)
{
	...
	for (const texture &tex : _textures)
	{
		if (tex.impl == nullptr ||
			...
			continue;
		const auto tex_impl = static_cast<d3d11_tex_data *>(tex.impl);

		// Update references in technique list
		for (const technique &tech : _techniques)
		{
			const auto tech_impl = static_cast<d3d11_technique_data *>(tech.impl);
