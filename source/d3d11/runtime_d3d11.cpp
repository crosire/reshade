...
namespace reshade::d3d11
{
	...
	void runtime_d3d11::capture_screenshot(uint8_t *buffer) const
	{
		...
		if (FAILED(_device->CreateTexture2D(&desc, nullptr, &intermediate)) ||
			FAILED(_immediate_context->Map(intermediate.get(), 0, D3D11_MAP_READ, 0, &mapped)))
		{
	...
	bool runtime_d3d11::init_texture(texture &info)
	{
	...
	void runtime_d3d11::update_texture(texture &texture, const uint8_t *pixels)
	{
		...
		case reshadefx::texture_format::rg8: {
			...
			break; }
		default:
			_immediate_context->UpdateSubresource(texture_impl->texture.get(), 0, nullptr, pixels, texture.width * 4, texture.width * texture.height * 4);
			break;
		}
	...
	bool runtime_d3d11::init_technique(technique &technique, d3d11_technique_data &&impl_init, const std::unordered_map<std::string, com_ptr<IUnknown>> &entry_points)
	{
		technique.impl = std::make_unique<d3d11_technique_data>(std::move(impl_init));
