...
void reshade::d3d11::buffer_detection::on_draw(UINT vertices)
{
	...
	if (_preserve_depth_buffers_hidden_by_rectangle && vertices == 6)
	{
		...
		if (dsv_texture != _context->_depthstencil_hidden_by_rectangle_index.first)
			return; // This is a draw call with no depth-stencil bound
...
#if RESHADE_DEPTH
void reshade::d3d11::buffer_detection::on_set_render_targets(ID3D11DepthStencilView *dsv)
{
