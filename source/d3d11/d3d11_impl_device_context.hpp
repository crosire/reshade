...
namespace reshade::d3d11
{
	...
	public:
		...
		void begin_render_pass(uint32_t, const api::render_pass_render_target_desc *, const api::render_pass_depth_stencil_desc *, api::render_pass_flags) final { assert(false); }
		...
		void resolve_texture_region(api::resource, uint32_t, const api::subresource_box *, api::resource, uint32_t, uint32_t, uint32_t, uint32_t, api::format) final { assert(false); }
		...
		void update_buffer_region(const void *, api::resource, uint64_t, uint64_t) final { assert(false); }
		void update_texture_region(const api::subresource_data &, api::resource, uint32_t, const api::subresource_box *) final { assert(false); }
