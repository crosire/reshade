...
HOOK_EXPORT PROC WINAPI wglGetProcAddress(LPCSTR lpszProc)
{
	...
	else if (static bool s_hooks_not_installed = true; s_hooks_not_installed)
	{
		...
		reshade::hooks::install("wglGetExtensionsStringARB", reinterpret_cast<reshade::hook::address>(trampoline("wglGetExtensionsStringARB")), reinterpret_cast<reshade::hook::address>(&wglGetExtensionsStringARB), true);
