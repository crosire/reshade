...
namespace reshade::opengl
{
	...
	void runtime_opengl::capture_screenshot(uint8_t *buffer) const
	{
		glBindFramebuffer(GL_READ_FRAMEBUFFER, 0);
		glReadBuffer(GL_BACK);
	...
	void runtime_opengl::update_texture(texture &texture, const uint8_t *pixels)
	{
	...
	bool runtime_opengl::init_technique(technique &technique, opengl_technique_data &&impl_init, const std::unordered_map<std::string, GLuint> &entry_points, std::string &errors)
	{
