...
void STDMETHODCALLTYPE D3D12GraphicsCommandList::CopyTextureRegion(const D3D12_TEXTURE_COPY_LOCATION *pDst, UINT DstX, UINT DstY, UINT DstZ, const D3D12_TEXTURE_COPY_LOCATION *pSrc, const D3D12_BOX *pSrcBox)
{
	...
	if (pSrc->Type == D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX && pDst->Type == D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX)
	{
		...
		if (pSrcBox != nullptr)
		{
			extent[0] = pSrcBox->right - pSrcBox->left;
			extent[1] = pSrcBox->bottom - pSrcBox->top;
			extent[2] = pSrcBox->back - pSrcBox->front;
		}
		...
		if (reshade::invoke_addon_event<reshade::addon_event::copy_texture_region>(this,
			...
			reshade::api::resource { reinterpret_cast<uintptr_t>(pDst->pResource) }, pDst->SubresourceIndex, DstX != 0 || DstY != 0 || DstZ != 0 ? dst_box : nullptr, pSrcBox != nullptr ? extent : nullptr))
			return;
	}
...
#else
	static_assert(sizeof(*ppDescriptorHeaps) == sizeof(reshade::api::descriptor_heap));
	const auto heap_handles = reinterpret_cast<const reshade::api::descriptor_heap *>(ppDescriptorHeaps);
#endif
...
void STDMETHODCALLTYPE D3D12GraphicsCommandList::SetComputeRootDescriptorTable(UINT RootParameterIndex, D3D12_GPU_DESCRIPTOR_HANDLE BaseDescriptor)
{
...
#if RESHADE_ADDON
	static_assert(sizeof(BaseDescriptor) == sizeof(reshade::api::descriptor_table));
	...
	reshade::invoke_addon_event<reshade::addon_event::bind_descriptor_tables>(
		...
		RootParameterIndex,
		1,
		reinterpret_cast<const reshade::api::descriptor_table *>(&BaseDescriptor.ptr));
#endif
}
...
{
#if RESHADE_ADDON
	if (reshade::invoke_addon_event<reshade::addon_event::clear_depth_stencil_view>(this, reshade::api::resource_view { DepthStencilView.ptr }, static_cast<uint32_t>(ClearFlags), Depth, Stencil))
		return;
#endif
...
{
#if RESHADE_ADDON
	if (const reshade::api::resource_view rtv = { RenderTargetView.ptr };
...
{
#if RESHADE_ADDON
	if (reshade::invoke_addon_event<reshade::addon_event::clear_unordered_access_view_uint>(this, reshade::api::resource_view { ViewCPUHandle.ptr }, Values))
		return;
#endif
...
{
#if RESHADE_ADDON
	if (reshade::invoke_addon_event<reshade::addon_event::clear_unordered_access_view_float>(this, reshade::api::resource_view { ViewCPUHandle.ptr }, Values))
		return;
#endif
...
{
#if RESHADE_ADDON
	if (reshade::invoke_addon_event<reshade::addon_event::draw_or_dispatch_indirect>(this, reshade::addon_event::draw_or_dispatch_indirect, reshade::api::resource { reinterpret_cast<uintptr_t>(pArgumentBuffer) }, ArgumentBufferOffset, MaxCommandCount, 0))
		return;
#endif
...
void STDMETHODCALLTYPE D3D12GraphicsCommandList::BeginRenderPass(UINT NumRenderTargets, const D3D12_RENDER_PASS_RENDER_TARGET_DESC *pRenderTargets, const D3D12_RENDER_PASS_DEPTH_STENCIL_DESC *pDepthStencil, D3D12_RENDER_PASS_FLAGS Flags)
{
	...
	reshade::invoke_addon_event<reshade::addon_event::bind_render_targets_and_depth_stencil>(this, NumRenderTargets, rtvs, reshade::api::resource_view { pDepthStencil != nullptr ? pDepthStencil->cpuDescriptor.ptr : 0 });
