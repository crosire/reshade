...
namespace reshade
{
	...
	public:
		...
		void get(const std::string &section, const std::string &key, std::vector<std::filesystem::path> &paths) const
		{
			const auto it1 = _sections.find(section);
			if (it1 == _sections.end())
				return;
			...
			paths.clear();
			for (size_t i = 0; i < it2->second.data().size(); i++)
				paths.emplace_back(std::filesystem::u8path(it2->second.as<std::string>(i)));
		}
