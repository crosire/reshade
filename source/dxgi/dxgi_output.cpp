...
ULONG STDMETHODCALLTYPE DXGIOutput::AddRef() { _orig->AddRef(); return InterlockedIncrement(&_ref); }
ULONG STDMETHODCALLTYPE DXGIOutput::Release() {
	const ULONG ref = InterlockedDecrement(&_ref);
	if (ref != 0) { _orig->Release(); return ref; }
...
HRESULT STDMETHODCALLTYPE DXGIOutput::SetPrivateData(REFGUID Name, UINT DataSize, const void *pData) { return _orig->SetPrivateData(Name, DataSize, pData); }
HRESULT STDMETHODCALLTYPE DXGIOutput::SetPrivateDataInterface(REFGUID Name, const IUnknown *pUnknown) { return _orig->SetPrivateDataInterface(Name, pUnknown); }
HRESULT STDMETHODCALLTYPE DXGIOutput::GetPrivateData(REFGUID Name, UINT *pDataSize, void *pData) { return _orig->GetPrivateData(Name, pDataSize, pData); }
HRESULT STDMETHODCALLTYPE DXGIOutput::GetParent(REFIID riid, void **ppParent)
{
	...
	if (hr == S_OK)
	{
		...
		if (SUCCEEDED(parent_unknown->QueryInterface(&adapter)))
		{
			...
			{
#if RESHADE_VERBOSE_LOG
				reshade::log::message(reshade::log::level::debug, "DXGIOutput::GetParent returning IDXGIAdapter%hu object %p (%p).", adapter_proxy->_interface_version, adapter_proxy, adapter_proxy->_orig);
#endif
				*ppParent = adapter_proxy;
			}
			else
			{
				reshade::log::message(reshade::log::level::warning, "Unknown interface %s in DXGIOutput::GetParent.", reshade::log::iid_to_string(riid).c_str());
...
HRESULT STDMETHODCALLTYPE DXGIOutput::GetDesc(DXGI_OUTPUT_DESC *pDesc) { return _orig->GetDesc(pDesc); }
HRESULT STDMETHODCALLTYPE DXGIOutput::GetDisplayModeList(DXGI_FORMAT EnumFormat, UINT Flags, UINT *pNumModes, DXGI_MODE_DESC *pDesc) { return _orig->GetDisplayModeList(EnumFormat, Flags, pNumModes, pDesc); }
HRESULT STDMETHODCALLTYPE DXGIOutput::FindClosestMatchingMode(const DXGI_MODE_DESC *pModeToMatch, DXGI_MODE_DESC *pClosestMatch, IUnknown *pConcernedDevice) { return _orig->FindClosestMatchingMode(pModeToMatch, pClosestMatch, pConcernedDevice); }
HRESULT STDMETHODCALLTYPE DXGIOutput::WaitForVBlank(void) { return _orig->WaitForVBlank(); }
HRESULT STDMETHODCALLTYPE DXGIOutput::TakeOwnership(IUnknown *pDevice, BOOL Exclusive) { return _orig->TakeOwnership(pDevice, Exclusive); }
void STDMETHODCALLTYPE DXGIOutput::ReleaseOwnership(void) { _orig->ReleaseOwnership(); }
HRESULT STDMETHODCALLTYPE DXGIOutput::GetGammaControlCapabilities(DXGI_GAMMA_CONTROL_CAPABILITIES *pGammaCaps) { return _orig->GetGammaControlCapabilities(pGammaCaps); }
HRESULT STDMETHODCALLTYPE DXGIOutput::SetGammaControl(const DXGI_GAMMA_CONTROL *pArray) { return _orig->SetGammaControl(pArray); }
HRESULT STDMETHODCALLTYPE DXGIOutput::GetGammaControl(DXGI_GAMMA_CONTROL *pArray) { return _orig->GetGammaControl(pArray); }
HRESULT STDMETHODCALLTYPE DXGIOutput::SetDisplaySurface(IDXGISurface *pScanoutSurface) { return _orig->SetDisplaySurface(pScanoutSurface); }
HRESULT STDMETHODCALLTYPE DXGIOutput::GetDisplaySurfaceData(IDXGISurface *pDestination) { return _orig->GetDisplaySurfaceData(pDestination); }
HRESULT STDMETHODCALLTYPE DXGIOutput::GetFrameStatistics(DXGI_FRAME_STATISTICS *pStats) { return _orig->GetFrameStatistics(pStats); }
...
BOOL STDMETHODCALLTYPE DXGIOutput::SupportsOverlays(void) {
