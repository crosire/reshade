...
bool reshade::d3d10::device_impl::create_framebuffer(uint32_t count, const api::resource_view *rtvs, api::resource_view dsv, api::framebuffer *out)
{
	...
	result->count = count;
	for (UINT i = 0; i < count; ++i)
		result->rtv[i] = reinterpret_cast<ID3D10RenderTargetView *>(rtvs[i].handle);
	result->dsv = reinterpret_cast<ID3D10DepthStencilView *>(dsv.handle);
