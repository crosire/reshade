...
namespace reshade::d3d10
{
	...
	bool draw_call_tracker::check_depth_texture_format(int format_index, ID3D10DepthStencilView *depthstencil)
	{
	...
	void draw_call_tracker::track_rendertargets(int format_index, ID3D10DepthStencilView *depthstencil, UINT num_views, ID3D10RenderTargetView *const *views)
	{
		assert(depthstencil != nullptr);
	...
	void draw_call_tracker::track_depth_texture(int format_index, UINT index, com_ptr<ID3D10Texture2D> src_texture, com_ptr<ID3D10DepthStencilView> src_depthstencil, com_ptr<ID3D10Texture2D> dest_texture, bool cleared)
	{
