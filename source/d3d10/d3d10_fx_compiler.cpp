...
namespace reshade
{
	namespace
	{
		...
		DXGI_FORMAT literal_to_format(unsigned int value, texture::pixelformat &name)
		{
			switch (value)
			{
	...
	d3d10_fx_compiler::d3d10_fx_compiler(d3d10_runtime *runtime, const fx::nodetree &ast, std::string &errors, bool skipoptimization) : compiler(ast, errors), _runtime(runtime), _skip_shader_optimization(skipoptimization), _is_in_parameter_block(false), _is_in_function_block(false), _current_global_size(0)
	{
	}
