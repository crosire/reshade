...
const pipeline_state reshade::d3d10::pipeline_states_blend[] = {
	// D3D10_BLEND_DESC::AlphaToCoverageEnable
	pipeline_state::sample_alpha_to_coverage,
	// D3D10_BLEND_DESC::BlendEnable[0]
	pipeline_state::blend,
	// D3D10_BLEND_DESC::SrcBlend
	pipeline_state::blend_color_src,
	// D3D10_BLEND_DESC::DestBlend
	pipeline_state::blend_color_dest,
	// D3D10_BLEND_DESC::BlendOp
	pipeline_state::blend_color_op,
	// D3D10_BLEND_DESC::SrcBlendAlpha
	pipeline_state::blend_alpha_src,
	// D3D10_BLEND_DESC::DestBlendAlpha
	pipeline_state::blend_alpha_dest,
	// D3D10_BLEND_DESC::BlendOpAlpha
	pipeline_state::blend_alpha_op,
	// D3D10_BLEND_DESC::RenderTargetWriteMask[0]
...
const pipeline_state reshade::d3d10::pipeline_states_rasterizer[] = {
	// D3D10_RASTERIZER_DESC::FillMode
	pipeline_state::fill_mode,
	// D3D10_RASTERIZER_DESC::CullMode
	pipeline_state::cull_mode,
	// D3D10_RASTERIZER_DESC::FrontCounterClockwise
	pipeline_state::front_face_ccw,
	// D3D10_RASTERIZER_DESC::DepthBias
	pipeline_state::depth_bias,
	// D3D10_RASTERIZER_DESC::DepthBiasClamp
	pipeline_state::depth_bias_clamp,
	// D3D10_RASTERIZER_DESC::SlopeScaledDepthBias
	pipeline_state::depth_bias_slope_scaled,
	// D3D10_RASTERIZER_DESC::DepthClipEnable
	pipeline_state::depth_clip,
	// D3D10_RASTERIZER_DESC::ScissorEnable
	pipeline_state::scissor_test,
	// D3D10_RASTERIZER_DESC::MultisampleEnable
	pipeline_state::multisample,
	// D3D10_RASTERIZER_DESC::AntialiasedLineEnable
	pipeline_state::antialiased_line,
};
const pipeline_state reshade::d3d10::pipeline_states_depth_stencil[] = {
	// D3D10_DEPTH_STENCIL_DESC::DepthEnable
	pipeline_state::depth_test,
	// D3D10_DEPTH_STENCIL_DESC::DepthWriteMask
	pipeline_state::depth_write_mask,
	// D3D10_DEPTH_STENCIL_DESC::DepthFunc
	pipeline_state::depth_func,
	// D3D10_DEPTH_STENCIL_DESC::StencilEnable
	pipeline_state::stencil_test,
	// D3D10_DEPTH_STENCIL_DESC::StencilReadMask
	pipeline_state::stencil_read_mask,
	// D3D10_DEPTH_STENCIL_DESC::StencilWriteMask
	pipeline_state::stencil_write_mask,
	// D3D10_DEPTH_STENCIL_DESC::FrontFace::StencilFailOp
	pipeline_state::stencil_front_fail,
	// D3D10_DEPTH_STENCIL_DESC::FrontFace::StencilDepthFailOp
	pipeline_state::stencil_front_depth_fail,
	// D3D10_DEPTH_STENCIL_DESC::FrontFace::StencilPassOp
	pipeline_state::stencil_front_pass,
	// D3D10_DEPTH_STENCIL_DESC::FrontFace::StencilFunc
	pipeline_state::stencil_front_func,
	// D3D10_DEPTH_STENCIL_DESC::BackFace::StencilFailOp
	pipeline_state::stencil_back_fail,
	// D3D10_DEPTH_STENCIL_DESC::BackFace::StencilDepthFailOp
	pipeline_state::stencil_back_depth_fail,
	// D3D10_DEPTH_STENCIL_DESC::BackFace::StencilPassOp
	pipeline_state::stencil_back_pass,
	// D3D10_DEPTH_STENCIL_DESC::BackFace::StencilFunc
	pipeline_state::stencil_back_func,
	// StencilRef
	pipeline_state::stencil_ref,
};

void reshade::d3d10::fill_pipeline_state_values(ID3D10BlendState *state, const FLOAT factor[4], UINT sample_mask, uint32_t (&values)[ARRAYSIZE(pipeline_states_blend)])
{
	if (state != nullptr)
	{
		...
		values[1] = desc.BlendEnable[0];
		values[2] = desc.SrcBlend;
		values[3] = desc.DestBlend;
		values[4] = desc.BlendOp;
		values[5] = desc.SrcBlendAlpha;
		values[6] = desc.DestBlendAlpha;
		values[7] = desc.BlendOpAlpha;
		values[8] = desc.RenderTargetWriteMask[0];
	}
	else
	{
		values[0] = FALSE;
		values[1] = FALSE;
		values[2] = D3D10_BLEND_ONE;
		values[3] = D3D10_BLEND_ZERO;
		values[4] = D3D10_BLEND_OP_ADD;
		values[5] = D3D10_BLEND_ONE;
		values[6] = D3D10_BLEND_ZERO;
		values[7] = D3D10_BLEND_OP_ADD;
		values[8] = D3D10_COLOR_WRITE_ENABLE_ALL;
	}
...
void reshade::d3d10::fill_pipeline_state_values(ID3D10RasterizerState *state, uint32_t (&values)[ARRAYSIZE(pipeline_states_rasterizer)])
{
	if (state != nullptr)
	{
		// All fields in 'D3D10_RASTERIZER_DESC' are 4 bytes, so can just load them into the value array directly
		static_assert(sizeof(D3D10_RASTERIZER_DESC) == sizeof(values));

		state->GetDesc(reinterpret_cast<D3D10_RASTERIZER_DESC *>(values));
	}
	else
	{
		values[0] = D3D10_FILL_SOLID;	// D3D10_RASTERIZER_DESC::FillMode
		values[1] = D3D10_CULL_BACK;	// D3D10_RASTERIZER_DESC::CullMode
		values[2] = FALSE;				// D3D10_RASTERIZER_DESC::FrontCounterClockwise
		values[3] = 0;					// D3D10_RASTERIZER_DESC::DepthBias
		values[4] = 0;					// D3D10_RASTERIZER_DESC::DepthBiasClamp
		values[5] = 0;					// D3D10_RASTERIZER_DESC::SlopeScaledDepthBias
		values[6] = TRUE;				// D3D10_RASTERIZER_DESC::DepthClipEnable
		values[7] = FALSE;				// D3D10_RASTERIZER_DESC::ScissorEnable
		values[8] = FALSE;				// D3D10_RASTERIZER_DESC::MultisampleEnable
		values[9] = FALSE;				// D3D10_RASTERIZER_DESC::AntialiasedLineEnable
	}
}
void reshade::d3d10::fill_pipeline_state_values(ID3D10DepthStencilState *state, UINT ref, uint32_t (&values)[ARRAYSIZE(pipeline_states_depth_stencil)])
{
	if (state != nullptr)
	{
	...
	{
		values[0] = TRUE;
		values[1] = D3D10_DEPTH_WRITE_MASK_ALL;
		values[2] = D3D10_COMPARISON_LESS;
		values[3] = FALSE;
		values[4] = D3D10_DEFAULT_STENCIL_READ_MASK;
		values[5] = D3D10_DEFAULT_STENCIL_WRITE_MASK;
		values[6] = values[10] = D3D10_STENCIL_OP_KEEP;
		values[7] = values[11] = D3D10_STENCIL_OP_KEEP;
		values[8] = values[12] = D3D10_STENCIL_OP_KEEP;
		values[9] = values[13] = D3D10_COMPARISON_ALWAYS;
	}

	values[14] = ref;
}

static void convert_memory_heap_to_d3d_usage(memory_heap heap, D3D10_USAGE &usage, UINT &cpu_access_flags)
{
	switch (heap)
	{
...
static void convert_d3d_usage_to_memory_heap(D3D10_USAGE usage, memory_heap &heap)
{
	switch (usage)
	{
...
void reshade::d3d10::convert_sampler_desc(const sampler_desc &desc, D3D10_SAMPLER_DESC &internal_desc)
{
	...
	internal_desc.ComparisonFunc = D3D10_COMPARISON_ALWAYS;
...
sampler_desc reshade::d3d10::convert_sampler_desc(const D3D10_SAMPLER_DESC &internal_desc)
{
	sampler_desc desc = {};
