...
namespace reshade::d3d10
{
	...
	void d3d10_draw_call_tracker::log_drawcalls(UINT drawcalls, UINT vertices)
	{
		_counters.vertices += vertices;
		_counters.drawcalls += drawcalls;
	}
	void d3d10_draw_call_tracker::log_drawcalls(ID3D10DepthStencilView* depthstencil, UINT drawcalls, UINT vertices)
	{
	...
	void d3d10_draw_call_tracker::track_depthstencil(ID3D10DepthStencilView *depthstencil, com_ptr<ID3D10Texture2D> texture)
	{
	...
	std::pair<ID3D10DepthStencilView *, ID3D10Texture2D *> d3d10_draw_call_tracker::find_best_depth_stencil(UINT width, UINT height, DXGI_FORMAT format)
	{
		const float aspect_ratio = float(width) / float(height);

		depthstencil_counter_info best_info = { };
		std::pair<ID3D10DepthStencilView *, ID3D10Texture2D *> best_match = { };
	...
	void d3d10_draw_call_tracker::track_depth_texture(UINT index, com_ptr<ID3D10Texture2D> src_texture, com_ptr<ID3D10DepthStencilView> src_depthstencil, com_ptr<ID3D10Texture2D> dest_texture, bool cleared)
	{
	...
	ID3D10Texture2D *d3d10_draw_call_tracker::find_best_cleared_depth_buffer_texture(DXGI_FORMAT format, UINT depth_buffer_clearing_number)
	{
