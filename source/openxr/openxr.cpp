...
on_vr_submit_d3d10(vr::IVRCompositor* compositor,
                   vr::EVREye eye,
                   ...
                   D3D10Device* device_proxy)
{
  ...
  // It is not valid to switch the texture type once submitted for the first
  // time
  else if (s_vr_swapchain->get_device() != device_proxy)
    return vr::VRCompositorError_InvalidTexture;

  if (!s_vr_swapchain->on_vr_submit(eye, { reinterpret_cast<uintptr_t>(texture) }, bounds, eye)) {
  ...
  if (eye != vr::Eye_Right) {
    ...
    reshade::invoke_addon_event<reshade::addon_event::present>(
      device_proxy, s_vr_swapchain, &left_rect, &left_rect, 0, nullptr);
#endif
    ...
    reshade::invoke_addon_event<reshade::addon_event::present>(
      device_proxy, s_vr_swapchain, &right_rect, &right_rect, 0, nullptr);
#endif
    ...
    // The left and right eye were copied side-by-side to a single texture in
    // 'on_vr_submit', so set bounds accordingly
...
static vr::EVRCompositorError
on_vr_submit_d3d11(vr::IVRCompositor* compositor,
                   vr::EVREye eye,
                                                        ...
                                                        vr::EVRSubmitFlags flags)> submit)
{
  ...
  texture->GetDevice(&device); // 'GetDevice' is at the same virtual function table index for
                               // 'ID3D10Texture2D' and 'ID3D11Texture2D', so this happens to
                               // work for either case

  // Check if the passed texture is actually a D3D10 texture
  // Cannot just 'QueryInterface' with 'ID3D10Texture2D', since that is
  // supported on D3D11 textures too for some reason, so instead check the
  // device
  if (com_ptr<ID3D10Device> device10; device->QueryInterface(&device10) == S_OK) {
    // Also check that the device has a proxy 'D3D10Device' interface, otherwise
    // it is likely still a D3D11 device exposing the 'ID3D10Device interface,
    // after 'ID3D11Device::CreateDeviceContextState' was called
    if (const auto device10_proxy = get_private_pointer_d3dx<D3D10Device>(device10.get()))
      ...
      return on_vr_submit_d3d10(
        compositor, eye, reinterpret_cast<ID3D10Texture2D*>(texture), bounds, flags, submit, device10_proxy);
  }
  ...
  if (eye != vr::Eye_Right) {
    ...
    const reshade::api::rect left_rect = s_vr_swapchain->get_eye_rect(eye);
    reshade::invoke_addon_event<reshade::addon_event::present>(
      device_proxy->_immediate_context, s_vr_swapchain, &left_rect, &left_rect, 0, nullptr);
#endif
    return vr::VRCompositorError_None;
  } else {
#if RESHADE_ADDON
    const reshade::api::rect right_rect = s_vr_swapchain->get_eye_rect(eye);
    reshade::invoke_addon_event<reshade::addon_event::present>(
      device_proxy->_immediate_context, s_vr_swapchain, &right_rect, &right_rect, 0, nullptr);
#endif
    s_vr_swapchain->on_present();
...
static vr::EVRCompositorError
on_vr_submit_d3d12(vr::IVRCompositor* compositor,
                   vr::EVREye eye,
                   const vr::D3D12TextureData_t* texture,
                   const vr::VRTextureBounds_t* bounds,
                   vr::EVRSubmitFlags flags,
                   std::function<vr::EVRCompositorError(vr::EVREye eye,
                                                        void* texture,
                                                        ...
                                                        vr::EVRSubmitFlags flags)> submit)
{
  ...
  // Resource should be in D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE state at
  // this point
  if (!s_vr_swapchain->on_vr_submit(eye, { reinterpret_cast<uintptr_t>(texture->m_pResource) }, bounds, eye)) {
    // Failed to initialize effect runtime or copy the eye texture, so submit
    // normally without applying effects
#if RESHADE_VERBOSE_LOG
    LOG(ERROR) << "Failed to initialize effect runtime or copy the eye texture for eye " << eye << '!';
  ...
  if (eye != vr::Eye_Right) {
    ...
    const reshade::api::rect left_rect = s_vr_swapchain->get_eye_rect(eye);
    reshade::invoke_addon_event<reshade::addon_event::present>(
      command_queue_proxy.get(), s_vr_swapchain, &left_rect, &left_rect, 0, nullptr);
#endif
    return vr::VRCompositorError_None;
  } else {
    // Synchronize access to the command queue while events are invoked and the
    // immediate command list may be accessed
    std::unique_lock<std::shared_mutex> lock(command_queue_proxy->_mutex);

#if RESHADE_ADDON
    const reshade::api::rect right_rect = s_vr_swapchain->get_eye_rect(eye);
    reshade::invoke_addon_event<reshade::addon_event::present>(
      command_queue_proxy.get(), s_vr_swapchain, &right_rect, &right_rect, 0, nullptr);
#endif
    s_vr_swapchain->on_present();
...
static vr::EVRCompositorError
on_vr_submit_opengl(vr::IVRCompositor* compositor,
                    vr::EVREye eye,
                    GLuint object,
                    const vr::VRTextureBounds_t* bounds,
                    vr::EVRSubmitFlags flags,
                                                         ...
                                                         vr::EVRSubmitFlags flags)> submit)
{
  ...
  if (g_current_context == nullptr)
    ...
    s_vr_swapchain
      = new reshade::openvr::swapchain_impl(g_current_context->get_device(), g_current_context, compositor);
  ...
  const reshade::api::resource eye_texture = reshade::opengl::make_resource_handle(
    (flags & vr::Submit_GlRenderBuffer) != 0
      ? GL_RENDERBUFFER
      : ((flags & vr::Submit_GlArrayTexture) != 0 ? GL_TEXTURE_2D_ARRAY : GL_TEXTURE_2D),
    object);

  if (!s_vr_swapchain->on_vr_submit(eye, eye_texture, bounds, eye)) {
    // Failed to initialize effect runtime or copy the eye texture, so submit
    // normally without applying effects
#if RESHADE_VERBOSE_LOG
    LOG(ERROR) << "Failed to initialize effect runtime or copy the eye texture for eye " << eye << '!';
  ...
  if (eye != vr::Eye_Right) {
    ...
    const reshade::api::rect left_rect = s_vr_swapchain->get_eye_rect(eye);
    reshade::invoke_addon_event<reshade::addon_event::present>(
      g_current_context, s_vr_swapchain, &left_rect, &left_rect, 0, nullptr);
#endif
    return vr::VRCompositorError_None;
  } else {
#if RESHADE_ADDON
    const reshade::api::rect right_rect = s_vr_swapchain->get_eye_rect(eye);
    reshade::invoke_addon_event<reshade::addon_event::present>(
      g_current_context, s_vr_swapchain, &right_rect, &right_rect, 0, nullptr);
#endif
    s_vr_swapchain->on_present();
...
#define VR_Interface_Impl(type, method_name, vtable_index, interface_version, impl, return_type, ...)                  \
  ...
  {                                                                                                                    \
    static const auto trampoline = reshade::hooks::call(type##_##method_name##_##interface_version,                    \
                                                        reshade::hooks::vtable_from_instance(pThis) + vtable_index);   \
    impl                                                                                                               \
  }
#define VR_Interface_Call(...) trampoline(pThis, ##__VA_ARGS__)
#else
// The OpenVR interface functions use the __thiscall calling convention on x86,
// so need to emulate that with __fastcall and a dummy second argument which
// passes along the EDX register value
#define VR_Interface_Impl(type, method_name, vtable_index, interface_version, impl, return_type, ...)                  \
  ...
  VR_Interface_Impl(
    ...
    008,
    {
      ...
      switch (eTextureType) {
        case vr::TextureType_DirectX: // API_DirectX
          return on_vr_submit_d3d11(
            pThis, eEye, static_cast<ID3D11Texture2D*>(pTexture), pBounds, nSubmitFlags, submit_lambda);
        case vr::TextureType_OpenGL: // API_OpenGL
          return on_vr_submit_opengl(pThis,
                                     eEye,
                                     static_cast<GLuint>(reinterpret_cast<uintptr_t>(pTexture)),
    ...
    void* pTexture,
    const vr::VRTextureBounds_t* pBounds,
    /* vr::VRSubmitFlags_t */ vr::EVRSubmitFlags nSubmitFlags)
    ...
    VR_Interface_Impl(
      ...
      009,
      {
        ...
        const auto submit_lambda
          = [&](vr::EVREye eye, void* handle, const vr::VRTextureBounds_t* bounds, vr::EVRSubmitFlags flags) {
              // The 'vr::Submit_TextureWithPose' and 'vr::Submit_TextureWithDepth'
              // flags did not exist in this OpenVR version yet, so can keep it
              // simple
        ...
        VR_Interface_Impl(
          ...
          001,
          {
            LOG(INFO) << "Redirecting "
                      << "IVRClientCore::Cleanup" << '(' << "this = " << pThis << ')' << " ...";
          ...
          VR_Interface_Impl(
            ...
            001,
            {
              ...
              LOG(INFO) << "Redirecting "
                        << "IVRClientCore::GetGenericInterface" << '(' << "this = " << pThis
                        << ", pchNameAndVersion = " << pchNameAndVersion << ')' << " ...";

              void* const interface_instance = VR_Interface_Call(pchNameAndVersion, peError);

              // Only install hooks once, for the first compositor interface
              // version encountered to avoid duplicated hooks This is necessary
              // because vrclient.dll may create an internal compositor instance
              // with a different version than the application to translate
              // older versions, which with hooks installed for both would cause
              // an infinite loop
              if (static unsigned int compositor_version = 0;
                  compositor_version == 0 && interface_instance != nullptr
                  && std::sscanf(pchNameAndVersion, "IVRCompositor_%u", &compositor_version)) {
                // The 'IVRCompositor::Submit' function definition has been
                // stable and has had the same virtual function table index
                // since the OpenVR 1.0 release (which was at
                // 'IVRCompositor_015')
                if (compositor_version >= 12)
                  reshade::hooks::install(
                    "IVRCompositor::Submit",
                    reshade::hooks::vtable_from_instance(static_cast<vr::IVRCompositor*>(interface_instance)),
                    5,
                    reinterpret_cast<reshade::hook::address>(&IVRCompositor_Submit_012));
                else if (compositor_version >= 9)
                  reshade::hooks::install(
                    "IVRCompositor::Submit",
                    reshade::hooks::vtable_from_instance(static_cast<vr::IVRCompositor*>(interface_instance)),
                    4,
                    reinterpret_cast<reshade::hook::address>(&IVRCompositor_Submit_009));
                else if (compositor_version == 8)
                  reshade::hooks::install(
                    "IVRCompositor::Submit",
                    reshade::hooks::vtable_from_instance(static_cast<vr::IVRCompositor*>(interface_instance)),
                    6,
                    reinterpret_cast<reshade::hook::address>(&IVRCompositor_Submit_008));
                else if (compositor_version == 7)
                  reshade::hooks::install(
                    "IVRCompositor::Submit",
                    reshade::hooks::vtable_from_instance(static_cast<vr::IVRCompositor*>(interface_instance)),
                    6,
                    reinterpret_cast<reshade::hook::address>(&IVRCompositor_Submit_007));
              }
...
VRClientCoreFactory(const char* pInterfaceName, int* pReturnCode)
{
  ...
  if (static unsigned int client_core_version = 0;
      ...
      && std::sscanf(pInterfaceName, "IVRClientCore_%u", &client_core_version)) {
    g_client_core = static_cast<vr::IVRClientCore*>(interface_instance);

    // The 'IVRClientCore::Cleanup' and 'IVRClientCore::GetGenericInterface'
    // functions did not change between 'IVRClientCore_001' and
    // 'IVRClientCore_003'
    reshade::hooks::install("IVRClientCore::Cleanup",
                            reshade::hooks::vtable_from_instance(static_cast<vr::IVRClientCore*>(interface_instance)),
                            1,
                            reinterpret_cast<reshade::hook::address>(&IVRClientCore_Cleanup_001));
    reshade::hooks::install("IVRClientCore::GetGenericInterface",
                            reshade::hooks::vtable_from_instance(static_cast<vr::IVRClientCore*>(interface_instance)),
                            3,
                            reinterpret_cast<reshade::hook::address>(&IVRClientCore_GetGenericInterface_001));
  }
...
check_and_init_openxr_hooks()
{
  ...
  VRClientCoreFactory(vr::IVRClientCore_Version, reinterpret_cast<int
*>(&error_code)); if (g_client_core != nullptr)
          g_client_core->GetGenericInterface(vr::IVRCompositor_Version,
&error_code);*/
}
