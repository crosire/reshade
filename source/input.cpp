...
static uint32_t s_last_desktop_change = 0;
bool is_desktop_current(uint32_t& timestamp) { return std::exchange(timestamp,s_last_desktop_change) == s_last_desktop_change; }
...
bool reshade::input::handle_window_message(const void *message_data)
{
	...
	bool is_display_message = details.message == WM_DISPLAYCHANGE;

	is_mouse_message = is_mouse_message || details.message == WM_MOUSELEAVE;

	// Ignore messages that are not related to mouse or keyboard input, or the display that owns this window
	if (details.message != WM_INPUT && !is_mouse_message && !is_keyboard_message && !is_display_message)
	{
		return false;
	}
	...
	{
		s_last_desktop_change = std::max(s_last_desktop_change, static_cast<uint32_t>(details.time));
		return false;
	}
	...
	if (is_mouse_message && details.message != WM_MOUSELEAVE)
	{
	...
	{
	case WM_MOUSELEAVE:
		input->_mouse_hovering = false;
		input->_mouse_tracking = false;
		break;
	case WM_INPUT:
		RAWINPUT raw_data;
...
void reshade::input::next_frame()
{
	...
	// Cursor position messages are only sent while hovered, negating the need for a WM_MOUSENTER TrackMouseEvent
	if (_mouse_position[0] != _last_mouse_position[0] ||
	    _mouse_position[1] != _last_mouse_position[1])
	{
	...
	if (_mouse_hovering)
	{
		...
		tme.hwndTrack = static_cast<HWND>(_window);
		...
		else
		{
			...
			if (TrackMouseEvent(&tme) && (tme.dwFlags & TME_LEAVE) == 0)
			{
				tme.dwFlags |= TME_LEAVE;
				if (!TrackMouseEvent(&tme))
				{
...
extern "C" HHOOK WINAPI HookSetWindowsHookExW(int idHook, HOOKPROC lpfn, HINSTANCE hmod, DWORD dwThreadId)
{
	...
	if (idHook == WH_KEYBOARD || idHook == WH_KEYBOARD_LL || idHook == WH_MOUSE || idHook == WH_MOUSE_LL)
