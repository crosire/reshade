...
%code requires
{
	...
	namespace ReShade
	{
		...
		public:
			...
			EffectTree::Index FoldConstantExpression(EffectTree::Index index) const;
	...
	| RULE_EXPRESSION_FUNCTION
	{
		if (parser.mAST[$1].Is<EffectNodes::Call>())
		{
			...
			if (intrinsic)
			{
				...
				@$ = @1, $$ = parser.FoldConstantExpression(node.Index);
			}
			else
			{
				@$ = @1, $$ = $1;
...
namespace ReShade
{
	...
	EffectTree::Index EffectParser::FoldConstantExpression(EffectTree::Index index) const
	{
		EffectNodes::Expression &expression = this->mAST[index].As<EffectNodes::Expression>(); \

		#pragma region Constant Folding
#define FOLD_UNARY(op) \
		if (this->mAST[expression.Operands[0]].Is<EffectNodes::Literal>()) \
		{ \
			EffectNodes::Literal &operand = this->mAST[expression.Operands[0]].As<EffectNodes::Literal>(); \
...
#define FOLD_BINARY(op) \
		if (this->mAST[expression.Operands[0]].Is<EffectNodes::Literal>() && this->mAST[expression.Operands[1]].Is<EffectNodes::Literal>()) \
		{ \
			EffectNodes::Literal &left = this->mAST[expression.Operands[0]].As<EffectNodes::Literal>(); \
			EffectNodes::Literal &right = this->mAST[expression.Operands[1]].As<EffectNodes::Literal>(); \
			union EffectNodes::Literal::Value result = { 0 }; \
...
#define FOLD_BINARY_FUNCTION(func) \
		...
		switch (expression.Operator)
		{
			...
			case EffectNodes::Expression::Cast:
				if (this->mAST[expression.Operands[0]].Is<EffectNodes::Literal>())
				{
					EffectNodes::Literal &node = this->mAST[expression.Operands[0]].As<EffectNodes::Literal>();
