...
namespace ReShade
{
	namespace
	{
		...
		public:
			...
			{			
				...
				srvdesc.Format = TypelessToLinearFormat(texdesc.Format);
				...
				srvdesc.Format = TypelessToSRGBFormat(texdesc.Format);
			...
			void Visit(const EffectNodes::Pass &node, std::vector<D3D11Technique::Pass> &passes)
			{
				...
				pass.Viewport.Width = static_cast<FLOAT>(rtdesc.Width);
				pass.Viewport.Height = static_cast<FLOAT>(rtdesc.Height);
	...
	D3D11Runtime::D3D11Runtime(ID3D11Device *device, IDXGISwapChain *swapchain) : mDevice(device), mSwapChain(swapchain), mImmediateContext(nullptr), mDeferredContext(nullptr), mBackBuffer(nullptr), mBackBufferTexture(nullptr), mBackBufferTargets(), mDepthStencil(nullptr), mDepthStencilReplacement(nullptr), mDepthStencilShaderResourceView(nullptr), mLost(true)
	{
	...
	bool D3D11Runtime::OnCreate(unsigned int width, unsigned int height)
	{
		...
		this->mSwapChain->GetDesc(&this->mSwapChainDesc);
		...
		bbdesc.SampleDesc = this->mSwapChainDesc.SampleDesc;
	...
	void D3D11Runtime::OnPresent()
	{
		...
		if (SUCCEEDED(this->mDeferredContext->FinishCommandList(FALSE, &list)))
		{
	...
	std::unique_ptr<Effect> D3D11Runtime::CreateEffect(const EffectTree &ast, std::string &errors) const
	{
		...
		dstdesc.SampleDesc = this->mSwapChainDesc.SampleDesc;
		...
		dssdesc.ViewDimension = dstdesc.SampleDesc.Count > 1 ? D3D11_SRV_DIMENSION_TEXTURE2DMS : D3D11_SRV_DIMENSION_TEXTURE2D;
		...
		if (FAILED(hr))
		{
			effect->mRasterizerState->Release();
		...
		{
			effect->mRasterizerState->Release();
			effect->mDepthStencilTexture->Release();
		...
		if (FAILED(hr))
		{
			effect->mRasterizerState->Release();
			effect->mDepthStencilTexture->Release();
			effect->mDepthStencilShaderResourceView->Release();
	...
	void D3D11Runtime::CreateScreenshot(unsigned char *buffer, std::size_t size) const
	{
		...
		if (this->mSwapChainDesc.SampleDesc.Count > 1)
		{
			...
			hr = this->mDevice->CreateTexture2D(&texdesc, nullptr, &textureResolve);

			if (SUCCEEDED(hr))
			{
				this->mImmediateContext->ResolveSubresource(textureResolve, 0, this->mBackBufferTexture, 0, texdesc.Format);
	...
	void D3D11Runtime::DetectBestDepthStencil()
	{
		...
		if (best != nullptr && this->mDepthStencil != best)
		{
			...
			if ((desc.BindFlags & D3D11_BIND_SHADER_RESOURCE) == 0 || desc.Format != DXGI_FORMAT_R24G8_TYPELESS)
			{
				texture->Release();
				texture = nullptr;
				...
				hr = this->mDevice->CreateTexture2D(&desc, nullptr, &texture);
				...
				if (SUCCEEDED(hr))
				{
					...
					ZeroMemory(&dsdesc, sizeof(D3D11_DEPTH_STENCIL_VIEW_DESC));
					dsdesc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
					dsdesc.ViewDimension = desc.SampleDesc.Count > 1 ? D3D11_DSV_DIMENSION_TEXTURE2DMS : D3D11_DSV_DIMENSION_TEXTURE2D;

					hr = this->mDevice->CreateDepthStencilView(texture, &dsdesc, &this->mDepthStencilReplacement);
				}
			}
			...
			if (SUCCEEDED(hr))
			{
				...
				this->mDevice->CreateShaderResourceView(texture, &srvdesc, &this->mDepthStencilShaderResourceView);
	...
	void D3D11Runtime::CreateDepthStencil(ID3D11Resource *resource, ID3D11DepthStencilView *depthstencil)
	{
	...
	D3D11Effect::D3D11Effect(std::shared_ptr<const D3D11Runtime> context) : mEffectContext(context), mRasterizerState(nullptr), mDepthStencil(nullptr), mDepthStencilTexture(nullptr), mDepthStencilShaderResourceView(nullptr), mConstantsDirty(true)
	{
	}
	...
	void D3D11Texture::UpdateFromColorBuffer()
	{
		...
		{
			this->mEffect->mEffectContext->mDeferredContext->ResolveSubresource(this->mTexture, 0, this->mEffect->mEffectContext->mBackBufferTexture, 0, D3D11EffectCompiler::TypelessToLinearFormat(desc.Format));
		}
	}
	void D3D11Texture::UpdateFromDepthBuffer()
	{
		if (this->mShaderResourceView[0] == this->mEffect->mEffectContext->mDepthStencilShaderResourceView || this->mEffect->mEffectContext->mDepthStencilShaderResourceView == nullptr)
		{
			return;
		}
		...
		ID3D11ShaderResourceView *previous[2] = { this->mShaderResourceView[0], this->mShaderResourceView[1] };
		...
		this->mShaderResourceView[1] = this->mEffect->mEffectContext->mDepthStencilShaderResourceView;
		...
		for (auto &technique : this->mEffect->mTechniques)
		{
			for (auto &pass : technique.second->mPasses)
			{
				for (auto &SR : pass.SR)
				{
					if (SR == previous[0] || SR == previous[1])
					{
	...
	bool D3D11Technique::Begin(unsigned int &passes) const
	{
		...
		context->ClearDepthStencilView(this->mEffect->mDepthStencil, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
	...
	void D3D11Technique::End() const
	{
		...
		// Reset rendertargets
		context->OMSetRenderTargets(1, &this->mEffect->mEffectContext->mBackBufferTargets[0], nullptr);
	}
