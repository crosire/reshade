...
uniform float3 Pencil_Color <
	ui_type = "color";
	ui_label = "Pencil Color [Paper]";
> = float3(0.156,0.3,0.738);
...
uniform float Pencil_Correction <
	...
	ui_max = 2.0;
	ui_step = 0.001;
	ui_label = "Pencil Correction [Paper]";
...
uniform float Intensity <
	...
	ui_max = 1.0;
	ui_step = 0.001;
	ui_label = "Pencil Intensity [Paper]";
...
uniform float Speed_Animation <
	...
	ui_max = 2.0;
	ui_step = 0.001;
	ui_label = "Speed Animation [Paper]";
...
uniform float Fade_With_Original <	
	...
	ui_max = 1.0;
	ui_step = 0.001;
	ui_label = "Lines Fade [Paper]";
	ui_tooltip = "How much does the lines blend with the paper.";
> = 1.0;

uniform float3 Back_Color <
	ui_type = "color";
	ui_label = "Paper Color [Paper]";
> = float3(1.0,1.0,1.0);

//SHADER START
texture tPaper <source="CameraFilterPack/CameraFilterPack_Paper1.jpg";> { Width=1024; Height=1024; Format = RGBA8;};
sampler2D sPaper { Texture=tPaper; MinFilter=LINEAR; MagFilter=LINEAR; MipFilter=LINEAR; AddressU = REPEAT; AddressV = REPEAT; };

uniform int _TimerX < source = "frametime"; >;

float4 PS_Paper(float4 pos : SV_Position, float2 texcoord : TEXCOORD0) : SV_Target
{
	float4 f = tex2D(ReShade::BackBuffer, texcoord);
	float3 paper = tex2D(sPaper,texcoord).rgb;
	float uv = texcoord.xy;
	float _TimeX;
	_TimeX += _TimerX;
	...
	float3 paper2 = tex2D(sPaper,texcoord+dist).rgb;
